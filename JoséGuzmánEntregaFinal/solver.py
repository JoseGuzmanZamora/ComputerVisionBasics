...
def make_grid(valores):
    """
    Division en 81 partes en base a los valores.
    """
...
def regreso(original, cuadrito, sudoku, zeros, matriz):
    """
    Imprimir el resultado de los numeros en la imagen original.

    Args:
        original(numpy array): imagen original 
        cuadrito(numpy array): imagen acortada del cuadro del sudoku
        sudoku(numpy array): arreglo con los 81 valores de resultado 
        zeros(array): arreglo con las posiciones en las que hay que pintar el resultado
        matriz(numpy array): matriz para aplicar la inversa y regresar el cuadrito
    Results:
        original(numpy array): imagen con los números pintados  
    """
    print(sudoku)
    division = 450 / 9
    divisionpeq = 450 / 81
    for i in range(len(zeros)):
        positiony = round((zeros[i][0] * division) + division - divisionpeq) 
        positionx = round(zeros[i][1] * division + (2*divisionpeq))
        #print(type(sudoku))
        value = sudoku[zeros[i][0],zeros[i][1]]
        cv.putText(cuadrito, str(value), (positionx,positiony),cv.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)

    inversa = np.linalg.inv(matriz)
    final = cv.warpPerspective(cuadrito, inversa, (original.shape[1],original.shape[0]))
    
    #por ultimo hay que sobreponerla
    for i in range(len(original)):
        for j in range(len(original[i])):
            if final[i,j][0] == 0 and final[i,j][1] == 0 and final[i,j][2] == 255:
...
if __name__ == '__main__':
    ...
    if(args.p):
        ...
        if imagen is None:
            ...
            mostrar(warped[0])
            grid_encontrado = find_grid(warped[0])
            numeros = predict(grid_encontrado[0])
            a_solucionar = make_grid([numeros,grid_encontrado[1]])
            print(a_solucionar)
            #solucion = solve_sudoku(a_solucionar)
            comoarray = np.asarray(prueba2)
            ceros = solve.findzeros(comoarray)
            resultado = solve.start(comoarray)
            if not isinstance(resultado,str):
                final = regreso(imagen, warped[0], resultado, ceros,warped[1])
                mostrar(final)
            else:
                print(resultado)
